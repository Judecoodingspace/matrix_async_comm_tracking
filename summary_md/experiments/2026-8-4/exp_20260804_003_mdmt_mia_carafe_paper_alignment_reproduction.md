...
- Every author invocation exposes exactly one `condition × pair` input directory.
- Local homography uses the previous valid transform when fewer than five local
  correspondences are available or RANSAC returns no matrix.
...
Formal complete: all 42 condition-pair runs finished. Paper-aligned MIA matches the
paper Table III tolerances on the 14-pair macro evaluation. The measurement gate still
requires explicit handling of the pair-55 extra prediction frame and a determinism audit.
Full MIA improves MDA over local/ID allocation but lowers MOT IDF1 and increases IDSW.
Freeze this synchronous baseline only after those two gate items are recorded; then
proceed to separate H, ID-state and supplementation delay ablations.
