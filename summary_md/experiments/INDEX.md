...
| `exp_20260731_002_matrix_real_embedding_quality_transfer` | 2026-07-31 | Frozen real embedding quality and tracking transfer | MATRIX `0-999` | GT projected bbox + LoS；M3OT-GeM/OSNet 双冻结模型；身份两折候选条件化校准；fixed_2/fixed_3 + 0.25m | `outputs/20260731_matrix_real_embedding_quality_transfer/` | pending run | Runner、缓存、进度心跳、checkpoint/resume 和测试已实现；等待用户手动 smoke/formal。 | Pending. 验证 simulated identity quality boundary 是否能预测真实 CNN embedding 的 fixed-lag 跟踪收益。Card: `summary_md/experiments/2026-7-31/exp_20260731_002_matrix_real_embedding_quality_transfer.md` |
