...
| `exp_20260817_001_mdmt_mia_candidate_compensation_onset_validation` | 2026-08-17 | 非测试数据上的候选集合补偿起点验证 | MDMT train/val frozen cohorts | Source-only strict XML→MDA GT protocol gate; no tracking/MVE | `outputs/20260817_mdmt_non_test_mda_gt_protocol/` | protocol gate failed | G1 passed (88 XML); G2 exact test equivalence passed 5/28 files only, with 347 source-derived extra rows and no `outside=1` test witness. G3-G6 were not run. | `GT_PROTOCOL_GATE_FAIL`; no MVE or experiment implementation. Only separately approved source annotation/export investigation may resume. Report: `summary_md/experiments/2026-8-17/exp_20260817_001_mdmt_mia_candidate_compensation_onset_validation/GT_PROTOCOL_GATE_REPORT.md` |
