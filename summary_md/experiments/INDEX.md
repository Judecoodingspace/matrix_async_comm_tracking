...
| `exp_20260817_001_mdmt_mia_candidate_compensation_onset_validation` | 2026-08-17 | 非测试数据上的候选集合补偿起点验证 | MDMT train/val frozen cohorts | `MDMT_SOURCE_ANNOTATION_MDA_V1`; isolated Pair53/66 MVE; frozen 15-pair development | `outputs/20260905_mdmt_mia_frozen_15_pair_development_v5/`; analysis `outputs/20260906_mdmt_mia_frozen_15_pair_development_analysis_v1/` | development full replication; d1 locked; holdout not run | ID-delay harm has positive CIs at d1--d5. d1--d4 pass Gate A--F; d5 fails only R_edge CI Gate A. The complete compensation path occurs in 15/15 pairs at every delay. | `EARLIEST_ONSET=d1`; `LOCK d1 AND PREPARE HOLDOUT CONFIRMATION`. No holdout/val run. Reports: `POST_DEVELOPMENT_EXECUTION_MEASUREMENT_VALIDITY_AUDIT.md`; `FROZEN_15_PAIR_DEVELOPMENT_SCIENTIFIC_ANALYSIS.md`. |
