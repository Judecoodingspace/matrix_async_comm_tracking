...
| `exp_20260803_002_mdmt_async_incremental_tracklet_fusion` | 2026-08-03 | MDMT 异步增量 Tracklet 融合 | MDMT val calibration -> official test | 单帧 latest packet 对比稳定 local ID + pooled appearance 增量 packet；arrival、timestamped、fixed-lag、late recovery | `outputs/20260803_mdmt_async_incremental_tracklet_fusion/` | planned | 代码、测试和 val-22 双向 smoke 完成；13 项实现门通过，单序列阈值样本不足，完整 Pilot 待运行。 | `implemented_pilot_pending`。Card: `summary_md/experiments/2026-8-3/exp_20260803_002_mdmt_async_incremental_tracklet_fusion.md` |
