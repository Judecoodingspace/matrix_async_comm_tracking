...
def validate(auth, evidence_loader=_git_show):
 ...
 if auth["previous_qualification_evidence_sha"]!=PREVIOUS_Q or auth["production_implementation_sha"]!=PRODUCTION or auth["execution_path_plan_authority"]!=PLAN or auth["contract_authority"]!=CONTRACT or auth["implementation_plan_authority"]!=IMPLEMENTATION_PLAN: raise GateError("frozen authority mismatch")
 ...
 evidence=json.loads(evidence_loader(q,"summary_md/communication/c5_execution_path_superseding_qualification_v2/C5_EXECUTION_PATH_QUALIFICATION_CONTEXT.json"))
 if evidence.get("execution_enabled_candidate_sha")!=c: raise GateError("qualification does not bind candidate")
 if not evidence.get("source_fingerprints"): raise GateError("missing immutable source fingerprints")
 for path, expected in evidence["source_fingerprints"].items():
  if hashlib.sha256((ROOT/path).read_bytes()).hexdigest()!=expected: raise GateError("BLOCK_SOURCE_IDENTITY_MISMATCH")
 return root
