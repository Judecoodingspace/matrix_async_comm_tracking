...
def validate(auth,evidence_loader=None):
 ...
 if evidence_loader is None: evidence_loader=_git_show
 try: evidence=json.loads(evidence_loader(qualification,QUALIFICATION_CONTEXT_PATH),object_pairs_hook=_object)
 ...
 if not isinstance(fingerprints,dict) or set(fingerprints)!=FROZEN_GOVERNED_FINGERPRINT_PATHS: raise GateError("governed source fingerprint set mismatch")
